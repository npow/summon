"""Click CLI: oneshot run / ideate / build subcommands."""
...
@main.command()
@click.argument("idea")
@click.option("--config", "-c", "config_path", default=None, help="Path to oneshot.yaml")
...
def run(
    ...
    Example: oneshot run "build a CLI tool that converts markdown to PDF"
    """
    console.print()
    console.print(Panel(
        f"[bold]{idea}[/bold]",
    ...
    config = OneshotConfig.load(config_path)
...
def ideate(
    ...
    Example: oneshot ideate "build a CLI tool that converts markdown to PDF"
    """
    console.print()
    console.print(Panel(
        f"[bold]{idea}[/bold]",
    ...
    try:
        ...
        project_name = spec.get("project_name", "oneshot-project")
        ...
        console.print(f"\n[dim]Next: oneshot plan {spec_path}[/dim]")
...
def plan(
    ...
    Example: oneshot plan my-tool.spec.json
    """
    ...
    try:
        ...
        console.print(f"\n[dim]Next: oneshot design {plan_path}[/dim]")
...
def design(
    ...
    Example: oneshot design my-tool.plan.json
    """
    ...
    try:
        ...
        console.print(f"\n[dim]Next: oneshot build {design_path}[/dim]")
...
def build(
    ...
    Example: oneshot build my-tool.design.json -o /tmp/my-tool
    """
    data = _load_json_file(spec_file)
