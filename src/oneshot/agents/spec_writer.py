...
from oneshot.prompts.stage1 import SELF_CLARIFY, SPEC_WRITER


def create_clarify_node(config: OneshotConfig):
...
def create_spec_writer_node(config: OneshotConfig):
    return create_agent_node(
        config=config,
        model_key="spec_writer",
