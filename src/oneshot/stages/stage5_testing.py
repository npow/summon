...
from oneshot.agents.integrator import create_integrator_node
from oneshot.agents.test_writer import create_test_writer_node
from oneshot.agents.bug_fixer import create_bug_fixer_node
from oneshot.agents.import_fixer import create_import_fixer_node
from oneshot.agents.code_regenerator import create_code_regenerator_node
from oneshot.agents.acceptance_criteria_gen import create_acceptance_criteria_gen_node
from oneshot.agents.acceptance_test_gen import create_acceptance_test_gen_node
from oneshot.agents.acceptance_fixer import create_acceptance_fixer_node
...
def _process_integration(state: dict[str, Any]) -> dict[str, Any]:
    ...
    import re
    from oneshot.workspace import normalize_file_entry
...
def _build_regen_context(state: dict[str, Any]) -> dict[str, Any]:
    ...
    healthy_parts = []
    for f in all_files:
        if f in degenerate_paths:
            continue
        try:
            content = ws.read_file(f)
            healthy_parts.append(f"=== {f} ===\n{content}")
        except Exception:
            pass
    ...
    if ws.file_exists("requirements.txt"):
        try:
            req = ws.read_file("requirements.txt")
            healthy_parts.append(f"=== requirements.txt ===\n{req}")
        except Exception:
            pass

    return {"healthy_files": "\n\n".join(healthy_parts) or "(no healthy files)"}
...
def _build_import_fix_context(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if workspace_path:
        ...
        if ws.file_exists("requirements.txt"):
            try:
                req_content = ws.read_file("requirements.txt")
                source_files += f"\n=== requirements.txt ===\n{req_content}\n"
            except Exception:
                pass

    return {"import_fix_source_files": source_files or "(no source files)"}
...
def create_stage5_graph(config: OneshotConfig) -> StateGraph:
