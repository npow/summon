...
from forge.agents.lld import create_lld_node
from forge.agents.coder import create_coder_node
from forge.agents.code_reviewer import create_code_reviewer_node
...
def create_component_graph(config: ForgeConfig) -> StateGraph:
    """Build the per-component subgraph: LLD → Code → Review loop.

    Uses ComponentState (not ForgeState) to avoid key conflicts when
...
def fan_out_components(state: dict[str, Any]) -> list[Send]:
    ...
    Only sends the keys defined in ComponentState — no extra ForgeState keys.
    """
...
def create_stage4_graph(config: ForgeConfig) -> StateGraph:
